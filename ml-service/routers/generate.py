...
from services.preprocessor import preprocess_tryon_images, image_to_bytes  
...
async def generate_tryon(request: BackendImagesUrlRequest):
    ...
    images = preprocess_tryon_images(self_image, garment_image)   # NEW — clean to RGB + enforce 768x1024
    
    result_url = await upload_result_image( 
        file_bytes=image_to_bytes(images.garment_image),
